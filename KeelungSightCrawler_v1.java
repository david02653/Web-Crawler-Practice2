...
public class KeelungSightCrawler {
    ...
    public void addDetail( String link, Sight temp ){
        ...
        try{
            Document web = Jsoup.connect(link).timeout(300000)
                                .userAgent("\"Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36")
                                .get();
            ...
            Elements getZone = web.select("span.point_type > span > strong");
            String zone = getZone.toString();
            ...
            Element img;
            if(web.getElementById("Piclink") == null){
                temp.setPhotoURL("none");
            }
            else{
                img = web.getElementById("Piclink");
                Elements imgUrl = img.getElementsByTag("img").select("img[src]");

                temp.setPhotoURL(imgUrl.attr("abs:src"));
            }
            ...
            Element googleMapLink = web.select("div.closing > a").first();
...
/* "http://travel.network.com.tw/tourguide/twnmap/keelungcity/" */
